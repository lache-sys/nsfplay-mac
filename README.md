...
## Restrictions  
本リポジトリでは、オリジナルの NSFPlay から VRC6 を除く拡張音源を削除する修正をしています。  
> 以下のコマンドを実行すれば全ての拡張音源に対応した状態にすることができます。  
> ```  
...
VRC6 も VGS 音源で代替可能なので削除しようと思いましたが、個人的に思い入れがあるので残しておきました。  
また、RaspberryPi Zero 2W のベアメタル環境で動作させるため、STL、浮動小数点数、プレイリスト機能、トラック情報機能などを使わない形に修正している影響で nsfconfig による設定の可変化を削除して、固定値の設定による nsfplay の改変モジュール（[src/nsfplay.hpp](src/nsfplay.hpp)）を用いて NSF の再生を行っています。（動作上の違いは無い...はず）  
## How to Execute  
